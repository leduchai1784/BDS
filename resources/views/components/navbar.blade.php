    ...
    >
        <div class="px-4 pt-2 pb-6 space-y-2">
            ...
            <div class="pt-4 border-t border-slate-100 flex flex-col space-y-2">
                @auth
                <div class="flex items-center space-x-3 px-3 py-2.5 mb-2 bg-slate-50 rounded-xl">
                    <img 
                        src="{{ Auth::user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=0077bb&color=fff' }}" 
                        alt="{{ Auth::user()->name }}" 
                    ...
                    >
                    <div class="text-left">
                        <span class="block text-sm font-extrabold text-slate-800 leading-none">{{ Auth::user()->name }}</span>
                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                            {{ Auth::user()->role === 'admin' ? 'Quản trị viên' : (Auth::user()->role === 'owner' ? 'Đối tác Chủ nhà' : 'Khách thuê nhà') }}
                        </span>
                    </div>
                </div>
